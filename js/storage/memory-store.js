var MemoryStore = function(successCallback, errorCallback) {
    ...
    this.findByName = function(searchKey, callback) {
		console.log('memory-store, findByName() med searchKey='+searchKey);
	// et element tilføjes employees[] hvis filter(funktionen returns true), filtreringsmekanisme indbygget i js-frameworket
        var employees = this.employees.filter(function(element) {
            var fullName = element.firstName + " " + element.lastName;
            return fullName.toLowerCase().indexOf(searchKey.toLowerCase()) > -1;
        }); // employees er datastruktur, array?, her, overføres så til callback funktionen:
        callLater(callback, employees);  
    }

	// id er unikt, så når når match fundet så afsluttes og callLater(callback, employee); kaldes, så fundet employee obj overføres sikker til callback-funktionen
    ...
    var callLater = function(callback, data) {
        if (callback) {
            setTimeout(function() {
                callback(data);
            });  // mærkeligt, Syntax fra js: setTimeout(function,milliseconds,lang)
        }
    }
    // ryan
    this.employees = [
    ...
    callLater(successCallback);  // kaldes vist når objektet er instansieret
